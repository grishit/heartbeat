...
export class Heartbeat {
  constructor(
    ...
    this.haveSomeValues = haveSomeValues;
    this.totalBPMValues = [];
    this.freqValues = [];
  }
  ...
  detectFace(gray) {
    ...
    if (faces.size() > 0) {
      ...
      decisionStatusEl.innerHTML = "Status - No Faces";
      this.invalidateFace();
    }
    faces.delete();
  }
  ...
  rppg() {
    ...
    if (this.signal.length >= this.targetFps * this.windowSize) {
      ...
      if (!signal.empty()) {
        ...
        this.totalBPMValues.push({ bpm, timestamp: Date.now() });

        if (this.totalBPMValues.length > 20 && !getResultsButton) {
          this.haveSomeValues();
          getResultsButton = true;
        }

        decisionStatusEl.innerHTML =
          "Status - Taking Readings, don't move. The longer you take reading the better results are produced.";
        // Draw BPM
        this.drawBPM(bpm);
      }
      signal.delete();
    } else {
      decisionStatusEl.innerHTML = "Status - Signal too small";
  ...
  drawFrequency(signal, low, high, bandMask) {
    ...
    console.log(result);
    this.freqValues.push({
      minValue: result.minVal,
